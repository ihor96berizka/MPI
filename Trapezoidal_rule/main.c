...
void Get_Input(int my_rank, int comm_sz, 
		double* a_p, double* b_p, int* n_p)
	{
		...
		if (my_rank == 0)
		{
			...
			{
				MPI_Send(a_p, 1, MPI_DOUBLE, dest, 0,
					 MPI_COMM_WORLD);
				MPI_Send(b_p, 1, MPI_DOUBLE, dest, 0,
					 MPI_COMM_WORLD);
				MPI_Send(n_p, 1, MPI_INT, dest, 0,
					 MPI_COMM_WORLD);
			}
		}
		else
...
int main (void)
{
	...
	double a = 0.0, b = 1 * M_PI, h, local_a, local_b;
	...
	if (my_rank == 0)
	{
		printf("With n = %d trapezoinds, out estimate\n", n);
